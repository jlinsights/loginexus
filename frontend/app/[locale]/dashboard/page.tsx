...
export default function ShipmentDashboard() {
  ...
  // Simulated: port disruption alerts for shipments with specific destinations
  const portDisruptionPorts = ['Shanghai', 'Busan', 'Rotterdam'];
  const portDisruptionShipments = shipments.filter(s =>
    s.current_status?.toUpperCase() !== 'DELIVERED' &&
    portDisruptionPorts.some(p => s.destination?.toLowerCase().includes(p.toLowerCase()))
  );
  ...
  const totalExceptions = delayedShipments.length + noLocationShipments.length +
    portDisruptionShipments.length + docPendingShipments.length;
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Shipment Dashboard</h1>
            ...
            {userRole === 'shipper' && (
                <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
                    ...
                    <button
                        ...
                        className={`p-2 rounded-md transition-all ${viewMode === 'table' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                    >
                        <List size={18} />
                    </button>
                </div>
      ...
      {userRole === 'shipper' ? (
        <>
            ...
            {totalExceptions > 0 && (
                <div className="bg-white rounded-2xl border border-[#E8E0D4] p-4 mb-6 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    <AlertTriangle size={16} className="text-amber-600" />
                    <h3 className="font-bold text-slate-800 text-sm">Exceptions & Alerts ({totalExceptions})</h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {delayedShipments.length > 0 && (
                    <div className="flex items-center gap-2.5 bg-red-50 rounded-xl px-3.5 py-3 border-l-4 border-red-400">
                        <Clock size={16} className="text-red-500 flex-shrink-0" />
                        <div className="flex-1">
                        <div className="text-xs font-bold text-slate-800">Delayed Shipments</div>
                        <div className="text-[11px] text-slate-500">{delayedShipments.length} shipment{delayedShipments.length > 1 ? 's' : ''} past ETA</div>
                        </div>
                        <span className="text-sm font-bold text-red-600">{delayedShipments.length}</span>
                    </div>
                    )}
                    {noLocationShipments.length > 0 && (
                    <div className="flex items-center gap-2.5 bg-amber-50 rounded-xl px-3.5 py-3 border-l-4 border-amber-400">
                        <MapPinOff size={16} className="text-amber-500 flex-shrink-0" />
                        <div className="flex-1">
                        <div className="text-xs font-bold text-slate-800">Missing Location</div>
                        <div className="text-[11px] text-slate-500">{noLocationShipments.length} without GPS data</div>
                        </div>
                        <span className="text-sm font-bold text-amber-600">{noLocationShipments.length}</span>
                    </div>
                    )}
                    {portDisruptionShipments.length > 0 && (
                    <div className="flex items-center gap-2.5 bg-sky-50 rounded-xl px-3.5 py-3 border-l-4 border-sky-400">
                        <CloudRain size={16} className="text-sky-500 flex-shrink-0" />
                        <div className="flex-1">
                        <div className="text-xs font-bold text-slate-800">Port Disruption</div>
                        <div className="text-[11px] text-slate-500">{portDisruptionShipments.length} affected by weather/congestion</div>
                        </div>
                        <span className="text-sm font-bold text-sky-600">{portDisruptionShipments.length}</span>
                    </div>
                    )}
                    {docPendingShipments.length > 0 && (
                    <div className="flex items-center gap-2.5 bg-violet-50 rounded-xl px-3.5 py-3 border-l-4 border-violet-400">
                        <FileWarning size={16} className="text-violet-500 flex-shrink-0" />
                        <div className="flex-1">
                        <div className="text-xs font-bold text-slate-800">Documentation Pending</div>
                        <div className="text-[11px] text-slate-500">{docPendingShipments.length} missing escrow/docs</div>
                        </div>
                        <span className="text-sm font-bold text-violet-600">{docPendingShipments.length}</span>
                    </div>
                    )}
                </div>
                </div>
            ...
            {/* Main Content Area */}
            {viewMode === 'map' ? (
                <div className="h-[600px] bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6 relative">
                ...
                <div className="absolute top-4 right-4 z-[400] flex flex-col gap-2 pointer-events-none">
                    <div className="bg-white/90 backdrop-blur-sm p-3 rounded-lg shadow-lg border border-slate-200/60 pointer-events-auto">
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1">
                        <CloudRain size={14} className="text-blue-400" /> Weather Alert
                    </div>
                    <div className="text-sm font-bold text-slate-800">Typhoon near Taiwan Strait</div>
                    </div>
                    <div className="bg-white/90 backdrop-blur-sm p-3 rounded-lg shadow-lg border border-slate-200/60 pointer-events-auto">
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1">
                        <FileWarning size={14} className="text-amber-500" /> Customs Delay
                    </div>
                    <div className="text-sm font-bold text-slate-800">2 Shipments in Busan</div>
                    </div>
                </div>
                </div>
            ) : (
                ...
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        ...
                        shipments.map((shipment) => (
                            <tr key={shipment.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => openMap(shipment)}>
                            <td className="px-6 py-4 font-bold text-slate-900">{shipment.tracking_number}</td>
                            ...
                            <td className="px-6 py-4 text-slate-500">{shipment.eta ? new Date(shipment.eta).toLocaleDateString() : '-'}</td>
                            <td className="px-6 py-4 text-right">
                                <span className="text-blue-600 font-medium text-xs opacity-0 group-hover:opacity-100 transition-opacity mr-3">
                                View Map
                                </span>
